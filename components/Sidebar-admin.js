...
export default function Sidebar() {
  ...
  const menuItems = [
      ...
      icon: (
        ...
        >
          <path
            ...
            strokeWidth="2"
            d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4m-3 0l2-2"
          />
        </svg>
      ),
    },
    ...
    {
      href: "/admin-dashboard/hasil",
      label: "hasil tes user",
      icon: (
        <svg
          className="w-5 h-5"
          fill="none"
  ...
  return (
    <>
      ...
      >
        {/* Header: Logo & Tombol Hamburger di KANAN ATAS */}
        ...
        {/* Menu Navigasi */}
        <nav className="mt-4 flex-1 px-2 overflow-y-auto max-h-[calc(100vh-160px)]">
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center px-3 py-2 text-sm rounded-lg mb-1 transition-all duration-200 group ${
                  isActive
      ...
      {/* Overlay Mobile (hanya saat sidebar terbuka) */}
      {isOpen && (
        <div
